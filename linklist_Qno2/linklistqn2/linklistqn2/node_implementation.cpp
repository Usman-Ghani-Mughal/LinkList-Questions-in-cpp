...
NodeImplementation NodeImplementation::splitting()
{
	int t_size=this->SIZE/2;
	Node *split_obj=this->START;
	NodeImplementation sec;
	sec.START=this->START;
	sec.SIZE++;
	this->SIZE--;
	Node *temp=sec.START;
	for(int i=1;i<t_size;i++)
	{
		...
		sec.SIZE++;
		this->SIZE--;
		split_obj=split_obj->NEXT;
	}
	this->START=split_obj->NEXT;
	temp->NEXT
		=NULL;
	return sec;
}